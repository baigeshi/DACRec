...
class Adversary(nn.Module):
    def __init__(self):
        ...
        # ml1m 400*128
        # steam2 30 *128
        self.linear_i = nn.Linear(400, 128)
        ...
        # ml1m=128*200
        # steam2=128*15
        self.linear_o = nn.Linear(128,100)
...
adversary = Adversary(hyper_params).to('cuda')
...
class BaseSolver:
    ...
    def solve(self) -> None:
        ...
        with open(os.path.join(self.data_dir, 'results_random.json')) as fp:
            ...
            line.append(results['MinSkew@10'])
            line.append(results['MaxSkew@10'])
            line.append(results['NDKL@10'])
    ...
    def adversary_kl_loss(self, adversary_prior, x_real, z_inferred, padding=None):

        prior = torch.randn_like(z_inferred.float())
        # print(x_real.size(),z_inferred.size())
        term_a = torch.log(torch.sigmoid(adversary_prior(x_real, z_inferred)) + 1e-9)
    ...
    def train_one_epoch(self, epoch: int) -> None:
        ...
        for batch in pbar:
            ...
            # get loss
            loss,tokens_adv,logits_adv= self.calculate_loss(batch)
            ...
            adver_kl_loss = self.adversary_kl_loss(adversary, tokens_adv.detach(), logits_adv.detach(), padding=None)
